...
echo "==> seed_collab.sql"
"${PSQL[@]}" -f "$DIR/seed_collab.sql"
echo "==> seed_product.sql"
"${PSQL[@]}" -f "$DIR/seed_product.sql"
echo "==> seed_discount.sql"
"${PSQL[@]}" -f "$DIR/seed_discount.sql"
echo "==> seed_company_relation.sql"
"${PSQL[@]}" -f "$DIR/seed_company_relation.sql"
echo "✅ 数据库结构与种子数据已就绪"
