...
export function ApiItemsPicker({ initial, onSave, onClose }: {
  ...
  const estAnnual = items.reduce((s, i) => s + i.quotePrice * i.estCalls, 0);
      ...
      footer={
        <>
          <span className="mr-auto text-sm text-text-weak">
            ...
            {estAnnual > 0 && <> · 预估年费用 <b className="text-primary">¥{estAnnual.toLocaleString('zh-CN', { maximumFractionDigits: 2 })}</b></>}
          </span>
